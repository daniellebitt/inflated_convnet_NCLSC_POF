...
#NB, the default if for this to build and load the 2-stream model using
#both the RGB and Flow streams, with weights pre-trained on imagenet. See multi_evaluate for how to run
#using just one stream and/or with/without imagenet pretraining
...
#remember, default: eval_type=joint and imaget_pretrained=True
...
#builds the model for the Flow stream and saves checkpoint
  ...
  #loads pre-trained checkpoints into tf session and pass sample into the 
  ...
  with tf.Session() as sess:
    ...
    #runs input through sample to make prediction
